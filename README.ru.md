...
### 4. Дыра в бюджете — «AI стоит как разработчик»

Agent-heavy фреймворки запускают 5-10 validator-субагентов на каждую задачу. Каждый субагент загружает контекст с нуля (30-40 тыс. токенов на вызов), делает проверку, возвращает summary. Умножь на 3 итерации fix'ов — одна micro-фича сжигает сотни тысяч токенов до того как написана первая строка кода. **На Claude-подписке это быстро съедает твои недельные rate limits. При оплате per token — стоит $10-20 за micro-фичу.**

**Ответ Trailmark:** все скиллы работают в твоём основном Claude-контексте — никаких fresh-context субагентов, которые дублируют загрузку файлов. `cadence-first` назначает Tier 1/2/3 на блок: тривиальный фикс идёт Plan-only (~$0.15 или минимум токенов), сложный рефакторинг — полная цепочка (~$1-2 или ~50 тыс. токенов). **В 5-10 раз меньше расхода токенов чем agent-heavy фреймворки — не упираешься в rate limits, не платишь $60 за micro-фичу.**
...
сессия продолжается с того места, где закончилась предыдущая, и ты не упираешься в rate limits и не сжигаешь $60 на micro-фичу.
...
- Каждый **скилл** *(переиспользуемый протокол типа `flow-first` или `plan-first`)* обязан создать настоящий файл (таблицу ландшафта, оценку объёма, документ решения, отчёт)
...
Trailmark поставляется с 19 скиллами, которые работают в **твоём основном Claude-контексте** — а не как изолированные субагенты, которые загружают 30-40 тыс. токенов на каждый вызов. Это осознанный архитектурный выбор, а не пропущенная фича:

- **Стоимость задачи падает в 5-10 раз** — никаких subagent-раздутий на тривиальной работе (Tier 3 блок ≈ $0.15)
- **Всё видно и дебажится** — все tool calls, edit'ы, проверки валидации в чате
- **Проще онбординг** — одна абстракция (скиллы), а не две (скиллы + агенты)
- **Right-size cadence** — `cadence-first` выбирает Tier 1/2/3 на блок вместо одного heavy pipeline на всё

Точечные агенты (deep research, parallel audit, security scan) добавим в поздних релизах как **opt-in power tools** — для тех 10% работы, где реально нужен параллелизм или изоляция контекста. Не как обязательный overhead на каждую задачу.
...
## Что это даёт поверх агента, который у вас уже есть

Честный вопрос до всего остального: Claude Code, Codex и Cursor и так читают
репозиторий, пишут файлы и запускают команды. Зачем сверху фреймворк?

Дело не в среднем результате — хороший агент чаще всего выдаёт хороший
результат. Дело в **разбросе** и в том, что остаётся после сессии. Спросите
дважды — получите две разные формы; вернитесь через месяц — рассуждения нет,
потому что единственной записью был чат.

Шесть вещей это меняют. Каждая — правило, которое скиллы соблюдают, а не
обещание:

**Заданный порядок вместо импровизации.** Понять ландшафт → проверить, что
уже есть → план → исполнение. Каждый шаг заканчивается файлом, а не
намерением, и следующий шаг не начнётся без файла предыдущего.

**Переиспользование прежде написания.** Один из шагов — буквально таблица:
по каждому куску работы, есть ли в репозитории что-то, что это уже
делает, и сколько новых строк, если нет. Написать своё быстрее, чем найти
чужое, поэтому без таблицы своё побеждает по умолчанию.

**Остановка перед необратимым.** Публикация, деплой, удаление — скилл встаёт
и ждёт человека. Не предупреждение по ходу, а именно остановка.

**Ритм, чтобы дисциплина оставалась по карману.** Не каждой правке нужен
полный цикл. Три уровня, и выбираются они до работы, а не по ходу. Без этого
протокол становится налогом на однострочные правки, а обложенный налогом
протокол начинают обходить.

**След, который можно спросить запросом.** Каждый артефакт кладёт строку
в файл SQLite. Благодаря этому вопрос «в каких блоках не хватает документа,
которого требовал их уровень» — настоящий запрос. В этом репозитории он
находил реальные пробелы в момент закрытия задачи.

**Уроки копятся там, где идёт работа.** Каждая ошибка, которую стоит помнить,
попадает в соответствующий скилл отдельным антипаттерном с именем. Следующий
прогон прочитает его прежде, чем повторить, — в том числе прогон в свежей
сессии, которая не помнит ничего.

### Чего это стоит

На однострочной правке всё это абсурд. Ровно поэтому существует выбор ритма
и отдельный короткий путь для срочных починок, который цикл не проходит вовсе.
Применять полный протокол везде — верный способ добиться того, чтобы его
перестали применять вообще.

### Чем это не является

Это не замена агенту — без него здесь не работает ничего. Это не улучшение
мышления модели: думает агент. И это не автономность — гейты существуют
затем, чтобы человек оставался в контуре там, где это важно.
...
| **Артефакты как настоящие файлы** | ❌ Только проза | ⚠️ Опционально / случайные trace | ❌ Эфемерный чат | ✅ Каждый шаг пишет отслеживаемый файл |
| **Память между блоками** | ❌ На человеке | ⚠️ Зависит от вашей обвязки | ❌ Только текущая сессия | ✅ Реестр переживает сессии |
| **Точки одобрения** | ❌ Этикет / текстовые правила | ⚠️ Хуки, которые вы вешаете | ❌ Доверие модели | ✅ Встроены в протокол скиллов |
...
| **Привязка** | Просто документ (никакого рантайма) | Рантайм + вендорский SDK | Только под редактор | Обычный markdown + bash + sqlite |
| **Язык скиллов** | Профессиональный жаргон | Python | Промпты модели | Обычный markdown, редактируется |
| **Архитектура** | Только проза | Runtime + agent-граф | Правила в редакторе | Скиллы в основном контексте, агенты по требованию (после релиза) |
| **Стоимость типичной задачи** | Не применимо (человеческий труд) | Средняя (зависит от custom кода) | Низкая (single-turn промпты) | **Низкая — $0.15-2/блок** |
...
рантайма, умеющего вызывать скиллы по имени) — есть экспериментальные
адаптеры под Codex (`adapters/codex/`) и Cursor (`adapters/cursor/`).
Сравнение выше — против категорий методологии, не против самого агентного
окружения. Claude Code — рантайм, на котором мы строимся, не конкурент.
О том, что фреймворк добавляет поверх этого рантайма, — раздел
[Что это даёт поверх агента, который у вас уже есть](#что-это-даёт-поверх-агента-который-у-вас-уже-есть)._
...
| **Разработчикам, которые ценят эффективность** — важно не упираться в rate limits подписки и не сжигать токены на agent-heavy overhead | Разработчикам небольших фич, где эффективность не важна; или тем, у кого неограниченный API-бюджет на токены |

Если ты один или в маленькой команде, делаешь инженерную работу с
агентом — и тебе важны **аудируемая история через сессии**, **эффективность
разработки** (5-10x меньше расхода токенов vs agent-heavy фреймворки — не
упираешься в rate limits подписки и не сжигаешь per-token бюджет), и
**дебажимый workflow** (скиллы в твоём основном контексте, не чёрный ящик
subagents) — ты в целевой аудитории.
...
│   └── .claude/
│       ├── skills/      # 18 скиллов (protocol + core)
...
graph TD
    ...
    T --> IF["idea-first<br/><i>новые продукты</i>"]
    T --> AF["arch-first<br/><i>новые фичи</i>"]
    T --> AUD["audit-first<br/><i>фиксы</i>"]
    IF --> B["Blocks<br/><i>N атомарных единиц</i>"]
    AF --> B
    AUD --> B
...
claude                               # или любой рантайм, вызывающий скиллы по имени
/go-start
...
**Текущий релиз — 20 скиллов + 1 инструмент, manifest v1.0.0.**
...
Живьём Trailmark запускался только на Claude Code. Рядом лежат два
**экспериментальных** адаптера — [`adapters/codex/`](./adapters/codex/) и
[`adapters/cursor/`](./adapters/cursor/): собраны генератором и вычитаны,
но ни один пока не запускался на живой установке. В README каждого написано,
что проверено, а что нет.

Третий адаптер — это профиль в `bin/build-adapter.sh`, а не второй генератор:
пять полей говорят, куда класть дерево, как вызывается скилл по имени, как
называется папка скиллов, чем запрещается неявный вызов и что написано
в маркере дерева. Шаги — в [`docs/AGENT_CONTRACT.md`](./docs/AGENT_CONTRACT.md).

Если запустишь любой из адаптеров — расскажи, как всё прошло, в
[Discussion #1](https://github.com/bearded-illirian/trailmark/discussions/1).
Отчёт с живой установки здесь ценнее, чем ещё одна заявка на платформу.
