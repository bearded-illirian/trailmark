...
Agent-heavy фреймворки запускают 5-10 validator-субагентов на каждую задачу. Каждый субагент загружает контекст с нуля (30-40 тыс. токенов на вызов), делает проверку, возвращает summary. Умножь на 3 итерации fix'ов — одна micro-фича сжигает $10-20 в API до того как написана первая строка кода.

**Ответ Trailmark:** все скиллы работают в твоём основном Claude-контексте — никаких fresh-context субагентов, которые дублируют загрузку файлов. `cadence-first` назначает Tier 1/2/3 на блок: тривиальный фикс идёт Plan-only (~$0.15), сложный рефакторинг — полная цепочка (~$1-2). **Типичный блок стоит $0.15-2. Неделя ежедневной работы: $5-10. В 5-10 раз дешевле agent-heavy фреймворков за ту же работу.**
...
сессия продолжается с того места, где закончилась предыдущая, и ты не сжигаешь $60 на micro-фичу.
...
| **Разработчикам, которые платят per token** и хотят предсказуемых затрат (~$5-10/неделя vs $60/micro-фича в других) | Командам с неограниченным API-бюджетом, кому не мешают validator-agent overhead |

Если ты один или в маленькой команде, делаешь инженерную работу с
агентом — и тебе важны **аудируемая история через сессии**, **предсказуемые
API-затраты** (~$5-10/неделя, а не $60 за micro-фичу) и **дебажимый workflow**
(скиллы в твоём основном контексте, не чёрный ящик subagents) — ты в
целевой аудитории.
