...
**Каждый артефакт цепочки скиллов — первоклассный файл, видимый в одном дереве.**

![Одна задача во Flow UI со всеми артефактами цепочки скиллов](docs/assets/flow-ui-task.png)

*Левая панель: сайдбар с каждым файлом что блок произвёл — flow-first, library-first, plan-first, report, user-note, decisions. Правая панель: markdown-preview. Никакого скрытого состояния, никакого прокручивания чата.*
...
**С одного взгляда видно какие проекты доминируют, какие типы задач самые частые, как распределяются типы артефактов.**

![Аналитика разбивки задач во Flow UI](docs/assets/flow-ui-tasks-breakdown.png)

*Когда каждый шаг пишет файл — вот что получается: реальный сигнал куда уходит время, какие оттенки работы чаще всего, какие типы артефактов фреймворк реально производит.*
...
**Каждый деплой ведёт к задаче, блоку и user-note что закрыл его.**

![Вкладка Deploys во Flow UI с привязкой commit → артефакт](docs/assets/flow-ui-deploys.png)

*Клик на любой деплой слева — правая панель открывает user-note написанный когда блок финишировал. Никакой охоты через git blame, никаких догадок какая регрессия из какого коммита.*
...
**Какие скиллы срабатывают чаще всего, какие спят, покрытие по доменам.**

![Дашборд методологии скиллов во Flow UI](docs/assets/flow-ui-skills.png)

*Ship-first возглавляет — потому что каждый блок им заканчивается. Спящие скиллы всплывают как кандидаты на курацию. Покрытие по доменам показывает какие срезы работы реально обслужены именованными протоколами.*
...
**Реальный масштаб — 18 тыс. артефактов в 1276 задачах, 5,5 тыс. деплоев, 6 доменов, 72 активных скилла.**

![Глобальная статистика с живого production-экземпляра Trailmark](docs/assets/flow-ui-global.png)

*Цифры с production-экземпляра автора. Trailmark — не игрушка, ежедневно двигает реальную инженерию на этом масштабе.*
