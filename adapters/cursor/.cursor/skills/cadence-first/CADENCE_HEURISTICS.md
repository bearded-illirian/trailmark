...
- **Not the skill itself.** SKILL.md at `aihub/.cursor/skills/cadence-first/SKILL.md` implements decision logic per these rules. Manifest = rulebook, SKILL = executor.
...
Any change to this manifest that touches rules — **§3 Golden Rule, §4 Anatomy (tier definitions), §5 Decision framework (Q1-Q5 + signals), §6 Anti-patterns, §8 Output format contract, §9 Legacy Migration** — MUST land in the **same commit** as any dependent SKILL.md changes in `aihub/.cursor/skills/cadence-first/SKILL.md`.
