...
Append `## Blocks` section to task.md. Output explicit "Registered N blocks: {list}. Launching $flow-first."
...
Choice 1 → `$flow-first` for first HIGH block. Choice 2 → `$dev-auto-first`.
...
Executed the protocol from memory — outdated version, no accuracy guarantee. Always via `$audit-first`.
